      ...
      <div class="card-body">
        ...
        <div class="row">
          <div class="form-group" style="margin-right: 10px">
            <?php echo Form::label('customer', 'Customer', ['class' => 'form-label']); ?>

            <?php echo Form::select('customer',$customers,$customer,['class'=>'form-control','placeholder'=>'Select Customer',$customerSelect]); ?>

          </div>
          <div class="form-group" style="margin-right: 10px">
            <?php echo Form::label('vendor', 'Vendor', ['class' => 'form-label']); ?>

            <?php echo Form::select('vendor',$vendors,$vendor,['class'=>'form-control','placeholder'=>'Select Vendor',$vendorSelect]); ?>
        ...
        <table class="table table-bordered table-striped table-hover"  id="">
          ...
          <tbody>
            <?php if(count($transactions)>0): ?>
                ...
                <tr>
                <td>
                    <input type="checkbox" name="ids[<?php echo e($row->id); ?>]" value="<?php echo e($row->id); ?>" class="checkbox" id="chk<?php echo e($row->id); ?>" onclick='checkcheckbox();' data-id="<?php echo e(base64_encode(number_format($row->remainingAmt,2,'.',''))); ?>" data-rem="<?php echo e(number_format($row->incExp->remaining,2,'.','')); ?>">
                    <input type="hidden" name="vals[<?php echo e($row->id); ?>]" value="<?php echo e(base64_encode(number_format($row->remainingAmt,2,'.',''))); ?>">
                </td>
                <td>
                ...
                <td><?php echo e(Helper::getCanonicalDateTime($row->date)); ?></td>
                <td>
                    <?php echo e(number_format($row->total,2,'.','')); ?>

                </td>
                <td class="remnants">
                  <?php echo e(number_format($row->incExp->remaining,2,'.','')); ?>

                  <input type="hidden" name="hid[<?php echo e($row->id); ?>]" class="hid" value="<?php echo e(number_format($row->incExp->remaining,2,'.','')); ?>">
                  <div class='input-group date'>
                    <div class="input-group-prepend" title="Full Amount">
                      <span class="input-group-text"><span class="fa fa-clipboard"></span></span>
                    </div>
                    <?php echo Form::text("price[$row->id]",null,['class'=>'form-control price','readonly','placeholder'=>'Price...','step'=>'any','onkeypress'=>'return isNumber(event,this)']); ?>
                ...
                <?php endif; ?>
                  <?php echo Form::select("faulty[$row->id]",$faulty,null,['class'=>'form-control faulty','placeholder'=>'Select','readonly','required']); ?>

                  <?php echo Form::textarea("remarks[$row->id]",null,['class'=>'form-control remarks','placeholder'=>'Remarks...','cols'=>'20','rows'=>'10']); ?>
          ...
          <tfoot>
            ...
            <tr> 
                <th colspan="3"></th>
                <th>
                <?php echo e(Hyvikk::get('currency')); ?> <?php echo e(number_format($totalAmount,2,'.','')); ?>

                
                <?php echo Form::hidden('total',base64_encode(number_format($remm,2,'.','')),['id'=>'total']); ?>

                <?php echo Form::hidden('amount',null,['id'=>'amount']); ?>

                </th>
                <th><?php echo e(Hyvikk::get('currency')); ?> <?php echo e(number_format($remm,2,'.','')); ?></th>
            ...
            <tr>
              ...
              <th colspan="2">
                ...
                <?php echo Form::text('date',date("Y-m-d H:i:s"),['class'=>'form-control','required']); ?>
...
$(document).ready(function() {
  ...
  $(".faulty").change(function(){
    if($(this).val()!="")
      $(this).closest("tr").find(".remarks").show();
    else
      $(this).closest("tr").find(".remarks").hide();
  })
        ...
        if(self.attr("id")=="chk_all"){ //all
          if($(this).prop("checked")==true){ //all checked
            $(".checkbox").each(function(){
              ...
              $(this).closest("tr").find(".price").prop({'readonly':false,'required':true});
              $(this).closest("tr").find(".faulty").attr('readonly',false).prop('required',true).css("pointer-events","");
            })
          }else{ //all unchecked
          $(".checkbox").each(function(){
              $(this).prop("checked",false);
              $(this).closest("tr").find(".price").prop({'readonly':true,'required':false}).val('');
              $(this).closest("tr").find(".faulty").attr('readonly',true).prop('required',false).css("pointer-events","none").val($(".faulty option:eq(0)").val()).change();
            })
          }
        }else{ //single
          if($(this).prop("checked")==true){ //single checked
            ...
            $(this).closest("tr").find(".price").prop({'readonly':true,'required':false}).val('');
            $(this).closest("tr").find(".faulty").attr('readonly',true).prop('required',false).css("pointer-events","none").val("").change();
          }
        }
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/VehicleMgmt/framework/resources/views/bank_account/bulk_pay.blade.php ENDPATH**/ ?>