    ...
    <div class="col-md-12">
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?php echo app('translator')->getFromJson('fleet.fuelReport'); ?></h3>
            </div>
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="card card-info mt-4">
            <div class="card-header">
                <h3 class="card-title">Own Stock Report</h3>
            </div>

            <div class="card-body table-responsive">
                <table class="table table-bordered table-striped table-hover" id="ownStockTable">
                    <thead>
                        <tr>
                            <th>SL#</th>
                            <th><?php echo app('translator')->getFromJson('fleet.date'); ?></th>
                ...
                <table class="table">
                    <tr>
                        <th style="float:right">Total Quantity: <?php echo e(bcdiv($own_stock_total_qty,1,2)); ?> ltr</th>
                    </tr>
                </table>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // Own Stock Table
        $('#ownStockTable tfoot th').each(function () {
            var title = $(this).text();
            $(this).html('<input type="text" placeholder="'+title+'" />');
        });
        
        var ownStockTable = $('#ownStockTable').DataTable({
            buttons: [{
                extend: 'collection',
                text: 'Export',
                buttons: [
            ...
            "initComplete": function() {
                ownStockTable.columns().every(function () {
                    var that = this;
                    $('input', this.footer()).on('keyup change', function () {
                        that.search(this.value).draw();
                    });
