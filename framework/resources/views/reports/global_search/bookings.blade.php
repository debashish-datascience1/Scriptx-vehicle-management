         ...
         <tr>
          ...
          <div class="btn-group">
            ...
            <div class="dropdown-menu custom" role="menu">
              <a class="dropdown-item vbook" data-id="{{$row->id}}" data-toggle="modal" data-target="#myModal2"> <span aria-hidden="true" class="fa fa-eye" style="color: #398439;"></span> @lang('fleet.viewBookingDetails')</a>
              ...
              @if($row->status==0 && $row->ride_status != "Cancelled" && !empty($row->transid) && $row->inc_rows<2 && Helper::isEligible($row->id,18))
                ...
                @if(empty($row->getMeta('advance_pay')))
              ...
              @if($row->vehicle_id != null)
                @if($row->status==0 && $row->receipt != 1)
                    ...
                    <a class="dropdown-item vcomplete" data-id="{{$row->id}}" data-toggle="modal" data-target="#modalComplete" data-backdrop='static' data-keyboard='false'> <span class="fa fa-check" aria-hidden="true" style="color: #0d9c00"></span> Mark as Complete</a>
