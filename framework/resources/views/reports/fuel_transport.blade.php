{{-- resources/views/reports/fuel_transport.blade.php --}}
@extends('layouts.app')
...
<div class="row">
    <div class="col-md-12">
        <div class="card card-info">
            ...
            <div class="card-body">
                {!! Form::open(['route' => 'reports.fuel-transport_post','method'=>'post','class'=>'form-block']) !!}
                <div class="row newrow">
                    <div class="col-md-3">
                        <div class="form-group">
                            {!! Form::label('transport', 'Select Transporter', ['class' => 'form-label']) !!}
                            {!! Form::select('transport_id',$transports,$request['transport_id'] ?? null,['class'=>'form-control transports fullsize','id'=>'transport_id','placeholder'=>'ALL']) !!}
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            {!! Form::label('fuel_type', __('fleet.fuelType'), ['class' => 'form-label']) !!}
                            {!! Form::select('fuel_type',$fuel_types,$request['fuel_type'] ?? null,['class'=>'form-control transports fullsize','id'=>'fuel_type','placeholder'=>'ALL']) !!}
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                ...
                <div class="row newrow">
                    <div class="col-md-12">
                        ...
                        <button type="submit" formaction="{{url('admin/print-fuel-transport-report')}}" formtarget="_blank" class="btn btn-danger"><i class="fa fa-print"></i> @lang('fleet.print')</button>
                    </div>
                </div>
                {!! Form::close() !!}
            </div>
...
@foreach($fuel as $k=>$f)
    <tr>
        ...
        <td><strong>{{$f->group_transport->group_name ?? 'N/A'}}</strong></td>
        <td>
            @if($f->vehicle_data)
                {{$f->vehicle_data->license_plate}}
            @else
                N/A
            @endif
        </td>
