          ...
          <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td nowrap>{{!empty($row->date) ? Helper::getCanonicalDate($row->date,'default') : Helper::getCanonicalDate($row->created_at,'default')}}
                    </td>
                    <td>
                        {{$row->pay_method->label}}
                    </td>
                    <td>
                        {{$row->params->label}}<br>
                        @if(in_array($row->advance_for,[21,22]) && $row->param_id==18)
                            {{$row->advancefor->label}}
                        @endif
                    </td>
                    <td>    
                        {{empty($row->bank) ? '-' : $row->bank->bank}}
                    </td>
                    <td>
                        {{$row->pay_type->label}}
                    </td>
                    ...
                    @if(in_array($row->advance_for,[21,22]))
                        <td>{{Helper::properDecimals($row->income_expense->amount)}}</td>
                    @else
                        <td>-</td>
                    @endif
                    <td>{{Helper::properDecimals($row->incExp->remaining)}}</td>
                    <td>
                        {{Helper::properDecimals($row->total)}} <br>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td><strong>{{Helper::properDecimals($totalTransaction)}}</strong></td>
                </tr>
            </tbody>
          </table>
        </div>
