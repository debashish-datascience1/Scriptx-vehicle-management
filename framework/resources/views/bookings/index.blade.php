    ...
    <div class="card card-info">
      <div class="card-header with-border">
        <h3 class="card-title"> @lang('fleet.manage_bookings') &nbsp;
          ...
          <a href="{{url("admin/refresh-json/18")}}" class="btn btn-primary refresh-table"><span class="fa fa-repeat"></span> &nbsp; @lang('fleet.refresh')</a>
            ...
            <tbody>
              @foreach($data as $k=>$row)
               <tr>
                ...
                <td>
                  <input type="checkbox" class="booking_checkbox" value="{{ $row->id }}" data-status="{{ $row->ride_status }}" data-advance="{{ $row->advance_pay ? 'yes' : 'no' }}">
                </td>
                ...
                <td style="width: 10% !important">
                    <div class="btn-group">
                      ...
                      <div class="dropdown-menu custom" role="menu">
                        ...
                        @if($row->ride_status == 'Completed')
                        <a class="dropdown-item vUndo" data-id="{{$row->id}}" style="cursor: pointer;">
                        ...
                        @if($row->ride_status != 'Completed')
                          <a class="dropdown-item vtype" data-id="{{$row->id}}" style="cursor:pointer;">
                            <span class="fa fa-trash" aria-hidden="true" style="color: #dd4b39;"></span> @lang('fleet.delete')
                          </a>
  ...
  $(document).ready(function() {
    $(".vUndo").click(function(){
        ...
        if(confirm("Are you sure you want to undo this completed booking?")) {
            ...
            $.ajax({
                url: '{{ url("admin/bookings") }}/' + id + '/undo-complete',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
...
$(document).ready(function() {
  ...
  $(document).on('click', '.bulk-action', function(e) {
    ...
    if (confirm('Are you sure you want to ' + action + ' the selected bookings?')) {
      $.ajax({
        url: '{{ route("bookings.bulk_action") }}',
        method: 'POST',
        data: {
          _token: '{{ csrf_token() }}',
          ids: selectedIds,
