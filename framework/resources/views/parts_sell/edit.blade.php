...
<li class="breadcrumb-item active">@lang('fleet.editPartsSale')</li>
@endsection
@section('content')
<div class="row">
  <div class="col-md-12">
    <div class="card card-success">
      <div class="card-header">
        <h3 class="card-title">@lang('fleet.editPartsSale')</h3>
      </div>
      ...
      <div class="card-body">
          ...
          {!! Form::model($data, ['route' => ['parts-sell.update', $data->id], 'method' => 'PATCH', 'id' => 'sellForm']) !!}
          
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  {!! Form::label('sell_to', __('fleet.sellTo'), ['class' => 'form-label']) !!}
                  {!! Form::text('sell_to', null, ['class' => 'form-control', 'required']) !!}
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  {!! Form::label('date', __('fleet.date'), ['class' => 'form-label']) !!}
                  {!! Form::text('date', $data->date_of_sell, ['class' => 'form-control datepicker', 'required']) !!}
                </div>
              </div>
            </div>

            <div id="item-rows">
              @foreach($allParts as $index => $item)
                <div class="item-row">
                  <div class="row">
                    <div class="col-md-3">
                      <div class="form-group">
                        {!! Form::label('item[]', __('fleet.item'), ['class' => 'form-label']) !!}
                        {!! Form::select('item[]', $items, $item->item, ['class' => 'form-control item', 'required']) !!}
                      </div> 
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        {!! Form::label('quantity[]', __('fleet.quantity'), ['class' => 'form-label']) !!}
                        {!! Form::number('quantity[]', $item->quantity, ['class' => 'form-control quantity', 'required', 'min' => '1']) !!}
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        {!! Form::label('tyre_numbers[]', __('fleet.selltyreNumbers'), ['class' => 'form-label']) !!}
                        {!! Form::text('tyre_numbers[]', $item->tyre_numbers, ['class' => 'form-control tyre_numbers', 'placeholder' => 'Enter comma-separated tyre numbers']) !!}
                        <input type="hidden" name="tyre_numbers_grouped[]" class="tyre_numbers_grouped" value="{{ $item->tyre_numbers }}">
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        {!! Form::label('amount[]', __('fleet.amount'), ['class' => 'form-label']) !!}
                        {!! Form::number('amount[]', $item->amount, ['class' => 'form-control amount', 'required', 'step' => '0.01', 'min' => '0']) !!}
                      </div>
                    </div>
                    <div class="col-md-1">
                      <div class="form-group">
                        {!! Form::label('total[]', __('fleet.total'), ['class' => 'form-label']) !!}
                        {!! Form::text('total[]', $item->total, ['class' => 'form-control total', 'readonly']) !!}
                      </div>
                    </div>
                    <div class="col-md-1">
                      <div class="form-group">
                        <label class="form-label">&nbsp;</label>
                        <button type="button" class="btn btn-danger remove-item" style="{{ $index == 0 ? 'display:none;' : '' }}">Remove</button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="row mt-4">
              <div class="col-md-offset-8 col-md-4">
                <div class="form-group">
                  ...
                  {!! Form::text('grand_total', $data->grand_total, ['class' => 'form-control', 'readonly', 'id' => 'grand_total']) !!}
                </div>
              </div>
            </div>
