@extends("layouts.app")
@section("breadcrumb")
<li class="breadcrumb-item active">@lang('fleet.deposit')</li>
@endsection
@section('extra_css')
        ...
        <table class="table" id="data_table">
          ...
          @foreach($deposits as $row)
            <tr>
              ...
              <td>
                @if($row->bank_id==1)
                  <span class="badge badge-primary">{{$row->bank->bank}}</span>
                @else
                  <label for="refer" class="form-label">{{$row->bank->bank}}</label>
                @endif
                @if(!empty($row->refer_bank))
                  <br>
                  <label for="refer" class="form-label">{{$row->referBank->bank}}</label>
                @endif
              </td>
              <td>{{Helper::getCanonicalDate($row->date,'default')}}</td>
              <td>{{Helper::properDecimals($row->amount)}}</td>
              <td>{{Helper::limitText($row->remarks,50)}}</td>
              <td>
              <div class="btn-group">
                ...
                <div class="dropdown-menu custom" role="menu">
                 ...
                 @if($row->from_id==null)
                  <a class="dropdown-item" href="{{url("admin/bank-account/deposit/".$row->id."/edit") }}"><span aria-hidden="true" class="fa fa-edit" style="color: #f0ad4e;"></span> @lang('fleet.edit')</a>
                @endif
...
<script type="text/javascript">
 $(".vevent").click(function(){
    var id = $(this).data("id");
    // console.log(id)
      $("#viewModal .modal-body").load('{{url("admin/bank-account/deposit/view_event")}}/'+id,function(res){
        // console.log(res)
