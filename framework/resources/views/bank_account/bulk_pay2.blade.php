        ...
        <div class="row">
          <div class="col-md-4 mb-2">
            <div class="form-group">
              ...
              {!!Form::select('vendor',$vendors,$vendor,['class'=>'form-control inputlen','placeholder'=>'Select Vendor','required',$vendorSelect])!!}
            </div>
          </div>
          <div class="col-md-4 mb-2">
            <div class="form-group">
        ...
        <table class="table table-bordered table-striped table-hover"  id="">
          ...
          <tbody>
            @if(count($transactions)>0)
                ...
                <td>{{Helper::getCanonicalDateTime($row->date,'default')}}</td>
                <td>
                    {{number_format($row->total,2,'.','')}}
                </td>
                <td class="remnants">
                  {{number_format($row->incExp->remaining,2,'.','')}}
                  <input type="hidden" name="hid[{{$row->id}}]" class="hid" value="{{number_format($row->incExp->remaining,2,'.','')}}">
                  <div class='input-group date'>
                    <div class="input-group-prepend" title="Full Amount">
                      <span class="input-group-text"><span class="fa fa-clipboard"></span></span>
                    </div>
                    {!!Form::text("price[$row->id]",null,['class'=>'form-control price','readonly','placeholder'=>'Price...','step'=>'any','onkeypress'=>'return isNumber(event,this)'])!!}<br>
                  </div>
                  <div class="method">
                    {!!Form::select("method[$row->id]",$method,null,['class'=>'form-control method','placeholder'=>'Select Method','readonly','required'])!!}
                    {!!Form::text("reference[$row->id]",null,['class'=>'form-control reference','readonly','placeholder'=>'Reference No..'])!!}
                  </div>
                  {{-- {!! Form::label('full_copy','Copy', ['class' => 'form-label']) !!} --}}
                </td>
                <td>
                @if($row->param_id==18)
                  {!! Form::label('driver',$row->booking->driver->name, ['class' => 'form-label']) !!}
                @endif
                  {!!Form::select("faulty[$row->id]",$faulty,null,['class'=>'form-control faulty','placeholder'=>'Select','readonly','required'])!!}
                  {!!Form::textarea("remarks[$row->id]",null,['class'=>'form-control remarks','placeholder'=>'Remarks...','cols'=>'20','rows'=>'10'])!!}
                </td>
          ...
          <tfoot>
            ...
            <tr> 
                ...
                <th>{{Hyvikk::get('currency')}} {{number_format($remm,2,'.','')}}</th>
        ...
        if(self.attr("id")=="chk_all"){ //all
          if($(this).prop("checked")==true){ //all checked
            $(".checkbox").each(function(){
              ...
              $(this).closest("tr").find(".reference").prop({'readonly':false,'required':true});
              $(this).closest("tr").find(".faulty").attr('readonly',false).prop('required',true).css("pointer-events","");
              $(this).closest("tr").find(".method").attr('readonly',false).prop('required',true).css("pointer-events","");
            })
          }else{ //all unchecked
          $(".checkbox").each(function(){
              $(this).prop("checked",false);
              $(this).closest("tr").find(".price").prop({'readonly':true,'required':false}).val('');
              $(this).closest("tr").find(".reference").prop({'readonly':true,'required':false}).val('');
              ...
              $(this).closest("tr").find(".method").attr('readonly',true).prop('required',false).css("pointer-events","none").val($(".method option:eq(0)").val()).change();
            })
          }
        }else{ //single
          if($(this).prop("checked")==true){ //single checked
            ...
            $(this).closest("tr").find(".method").attr('readonly',true).prop('required',false).css("pointer-events","none").val("");
          }
        }
