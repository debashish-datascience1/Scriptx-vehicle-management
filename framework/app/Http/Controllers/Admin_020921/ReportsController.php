	   ...
	   $from=Vendor::pluck('name','id');
		...
		foreach($transaction as $t){
			...
			elseif($t->param_id==26){
				$shalom = PartsModel::where('id',$t->from_id);
...
//	dd($filtered);
	...
	*/
	...
	public function fuel_post(Request $request) {
		...
		if (!empty($vehicle_id)) {
			$data['fuel'] = FuelModel::where('vehicle_id',$vehicle_id)->whereBetween('date', [$start, $end])->get();
		}
		else {
			$data['fuel'] = FuelModel::whereBetween('date', [$start, $end])->get();
		}

		// if($request->get('month') == '0' && $vehicle_id == ''){
		// 	$data['fuel'] = FuelModel::whereYear('date', $data['year_select'])->whereBetween('date', [$start, $end])->get();
		// }
		// else if ($request->get('month') == '0' ) {
		// 	$data['fuel'] = FuelModel::whereYear('date', $data['year_select'])->where('vehicle_id', $vehicle_id)->whereBetween('date', [$start, $end])->get();
		// }
		// else if ($vehicle_id == '') {
		// 	$data['fuel'] = FuelModel::whereYear('date', $data['year_select'])->whereMonth('date', $data['month_select'])->whereBetween('date', [$start, $end])->get();
		// }
		// else {
		// 	$data['fuel'] = FuelModel::whereYear('date', $data['year_select'])->whereMonth('date', $data['month_select'])->where('vehicle_id', $vehicle_id)->whereBetween('date', [$start, $end])->get();
		// }
		// $data['fuel'] = $data['fuel']->where('fuel_type','!=',3);
		// $data['lubs'] = $data['fuel']->where('fuel_type',3)->get()->sum('qty');
		// $data['lubs_price'] = $data['fuel']->where('fuel_type',3)->get()->sum('qty');
		// Total Price, Total Fuel
		foreach($data['fuel'] as $f){
			$f->total = $f->qty * $f->cost_per_unit;
		}
	...
	public function print_booking(Request $request) {
		...
		$data['year_select'] = $request->get("year");
		$data['month_select'] = $request->get("month");
		//$data['bookings'] = Bookings::whereMonth("pickup", $data['month_select'])->whereMonth("pickup", $data['month_select'])->whereIn('vehicle_id', $vehicle_ids)->groupBy('customer_id');
		if(empty($data['year_select']) && !empty($data['month_select']))
			$data['bookings'] = Bookings::whereMonth("pickup", $data['month_select'])->whereIn('vehicle_id', $vehicle_ids);
		elseif(!empty($data['year_select']) && empty($data['month_select']))
			$data['bookings'] = Bookings::whereYear("pickup", $data['year_select'])->whereIn('vehicle_id', $vehicle_ids);
