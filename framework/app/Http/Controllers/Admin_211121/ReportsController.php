...
class ReportsController extends Controller {
	...
	*/
	...
	public function statement_post(Request $request){
		// dd($request->all());
		$transaction = IncomeExpense::orderBy("id","ASC")->get();
		...
		$filtered = $transaction->where('dateof','!=',null)->flatten();
		if($request->get('date1')==null)
		...
		$openingCredit = $filtered->where('dateof','<',$start)->where('type',23)->sum('amount');
		$openingDebit = $filtered->where('dateof','<',$start)->where('type',24)->sum('amount');
		$openingBalance = $openingCredit-$openingDebit;
		// dd($openingCredit,$openingDebit,$openingBalance);

		$closingCredit = $filtered->where('dateof','<',$end)->where('type',23)->sum('amount');
		$closingDebit = $filtered->where('dateof','<',$end)->where('type',24)->sum('amount');
