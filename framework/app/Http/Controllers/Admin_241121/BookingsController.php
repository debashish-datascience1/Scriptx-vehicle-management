...
class BookingsController extends Controller
{
    ...
    public function index()
    {
        ...
        //for db load
        $data['data'] = Bookings::orderBy('id','DESC')->paginate(25);
    ...
    public function getDistanceFromAddress(Request $request){
        ...
        $from_url = "https://maps.googleapis.com/maps/api/geocode/json?address={$from_address}&key={$key}&sensor=false";
        $to_url = "https://maps.googleapis.com/maps/api/geocode/json?address={$to_address}&key={$key}&sensor=false";

        //Requesting google for latitude and longitude of an address
        return $from_response =  json_decode(file_get_contents($from_url),true);
    ...
    public function modal_save(Request $request){
        ...
        if(count($request->all())>2){
            ...
            foreach($inputs as $key=>$arr){
                ...
                // Accounting
                if(!empty($request->$arr) && $key==9){
                    $account['from_id'] = $id; //Advance Driver Id
                    $account['bank_id'] = 1;// Bank ID SELF CASH 
                    $account['type'] = 23; //Credit 
                    $account['param_id'] = 27; //From Refund [Mark as Complete]
                    $account['advance_for'] = null; //refunded, not advance needed
                    $account['from_transaction'] = 1;
                    $account['total'] = $request->$arr;
                    

                    $transid = Transaction::create($account)->id;
                    $trash = ['type'=>23,'from'=>27,'id'=>$transid];
                    $transaction_id = Helper::transaction_id($trash);
                    Transaction::find($transid)->update(['transaction_id'=>$transaction_id]);
