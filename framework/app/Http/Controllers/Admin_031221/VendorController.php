...
class VendorController extends Controller
{
    ...
    public function report_post(Request $request){
        ...
        if(empty($vendor) && empty($fuel_type))
            $fuelModel = FuelModel::withTrashed()->whereBetween('date',[$from_date,$to_date]);
        elseif(empty($vendor))
            $fuelModel = FuelModel::withTrashed()->whereBetween('date',[$from_date,$to_date])->where('fuel_type',$fuel_type);
        elseif(empty($fuel_type))
            $fuelModel = FuelModel::withTrashed()->whereBetween('date',[$from_date,$to_date])->where('vendor_name',$vendor);
        else
            $fuelModel = FuelModel::withTrashed()->whereBetween('date',[$from_date,$to_date])->where('vendor_name',$vendor)->where('fuel_type',$fuel_type);
