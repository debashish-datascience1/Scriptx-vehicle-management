...
class BankAccountController extends Controller
{
    ...
    public function bulk_paypost(Request $request){
        ...
        foreach($index['transactions'] as $k=>$t){
            if($t->param_id==18){ //Booking
                ...
                $t->date = $t->dateof = $shalom->exists() ? date("Y-m-d",strtotime($shalom->first()->created_at)) : null;
                $t->vc = !empty($t->org_id) ? Vendor::find($t->org_id)->name : null;
    ...
    public function bulk_store(Request $request){
        ...
        foreach($ogPrice as $id=>$price){
            ...
            $inEx = [
                'transaction_id'=>$id,
                'payment_method'=>$ogMethod[$id],
                'ref_no'=>$ogReference[$id],
                'date'=>$ogDate,
                'amount'=>$entered,
                'remaining'=>$remaining,
            ...
            if(in_array($ogAction[$id],[3,4]) && $remaining!=0){ //3->to self,4->driver's fault
                ...
                $income['transaction_id'] = $transid;
                $income['payment_method'] = $ogMethod[$id];//DD
                $income['ref_no'] = $ogReference[$id];//Reference
                $income['date'] = $ogDate;
