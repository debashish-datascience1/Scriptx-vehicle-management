            ...
            foreach($inputs as $key=>$arr){
                ...
                if(!empty($request->$arr) && ($key==7 || $key==9)){
                    ...
                    if($key==7){//Advance to Driver
                        $driver_id = Bookings::find($bookingid)->driver_id;
                        $array = ['driver_id'=>$driver_id,'date'=>date("Y-m-d"),'amount'=>bcdiv($request->$arr,1,2),'advance_driver_id'=>$id,'remarks'=>$remarks];
