...
class FuelController extends Controller {
	...
	public function store(FuelRequest $request) {
		...
		$fuel->cgst = $cgst;
		$fuel->sgst = $sgst;
		$fuel->cgst_amt = Helper::properDecimals($cgstval);
		$fuel->sgst_amt = Helper::properDecimals($sgstval);
		$fuel->total = Helper::properDecimals($total);
		$fuel->grand_total = Helper::properDecimals($grandtotal);
		...
		if(!empty($request->qty) && !empty($request->cost_per_unit)){
			...
			$account['total'] = Helper::properDecimals($grandtotal);
			...
			$income['amount'] = 0;
			$income['remaining'] = Helper::properDecimals($grandtotal);;
			$income['remarks'] = null;
	...
	public function report_vehicle(){
		$index['vehicles'] = FuelModel::select('id','vehicle_id')->groupBy('vehicle_id')->get();
		$index['fuel_types'] = FuelType::pluck('fuel_name','id');
		$index['request'] = null;
		// dd($index);
