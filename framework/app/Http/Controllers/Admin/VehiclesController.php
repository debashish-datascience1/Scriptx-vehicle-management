...
class VehiclesController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $purchase_info = PurchaseInfo::where('vehicle_id', $id)->first();
        $methods = Helper::getMethods();
    ...
    {
        // dd($request->all());
        // $abc = !empty($request->get("year")) ? Helper::ymd($request->get("year")) : null;
        // dd($abc);
    ...
    public function store_insurance(InsuranceRequest $request)
    {
        ...
        $vehicle->setMeta([
            ...
            'ins_exp_date' => !empty($request->get('exp_date')) ? Helper::ymd($request->get('exp_date')) : null,
            'ins_renew_duration' => $request->get('ins_renew_duration'),
            'insurance_duration_unit' => $request->get('insurance_duration_unit'),
            // 'ins_renew_amount' => $request->get('ins_renew_amount'),
            'fitness_tax' => $request->get('fitness_tax'),
            'fitness_expdate' => !empty($request->get('fitness_expdate')) ? Helper::ymd($request->get('fitness_expdate')) : null,
            'fitness_renew_duration' => $request->get('fitness_renew_duration'),
            'fitness_duration_unit' => $request->get('fitness_duration_unit'),
            // 'fitness_renew_amount' => $request->get('fitness_renew_amount'),
            'road_tax' => $request->get('road_tax'),
            'road_expdate' => !empty($request->get('road_expdate')) ? Helper::ymd($request->get('road_expdate')) : null,
            'roadtax_renew_duration' => $request->get('roadtax_renew_duration'),
            'roadtax_duration_unit' => $request->get('roadtax_duration_unit'),
            // 'roadtax_renew_amount' => $request->get('roadtax_renew_amount'),
            'permit_number' => $request->get('permit_number'),
            'permit_expdate' => !empty($request->get('permit_expdate')) ? Helper::ymd($request->get('permit_expdate')) : null,
            'permit_renew_duration' => $request->get('permit_renew_duration'),
            'permit_duration_unit' => $request->get('permit_duration_unit'),
            // 'permit_renew_amount' => $request->get('permit_renew_amount'),
            'pollution_tax' => $request->get('pollution_tax'),
            'pollution_expdate' => !empty($request->get('pollution_expdate')) ? Helper::ymd($request->get('pollution_expdate')) : null,
            'pollution_renew_duration' => $request->get('pollution_renew_duration'),
            'pollution_duration_unit' => $request->get('pollution_duration_unit'),
    ...
    public function updateVehicleInfo(Request $request)
    {
        ...
        $data_arr['loan_date'] = Helper::ymd($request->loan_date);
        $data_arr['emi_date'] = Helper::ymd($request->emi_date);
        ...
        $id = PurchaseInfo::updateOrCreate(['vehicle_id' => $request->vehicle_id], $data_arr)->id;

        //Prep for transaction
        $account['from_id'] = $id; //PurchaseInfo id
        $account['type'] = 24; //Debit
        $account['bank_id'] = $request->bank; //Bank Account
        $account['param_id'] = 49; //From id of Down Payment
        $account['is_completed'] = 1;
        $account['total'] = bcdiv($request->amount_paid, 1, 2);


        $tt = Transaction::updateOrCreate(['from_id' => $id, 'param_id' => 49], $account);
        if ($tt->wasRecentlyCreated) {
            $trash = ['type' => 24, 'from' => 49, 'id' => $tt->id];
            $transaction_id = Helper::transaction_id($trash);
            Transaction::find($tt->id)->update(['transaction_id' => $transaction_id]);
        }


        //New IncomeExpense Add
        $income['transaction_id'] = $tt->id;
        $income['payment_method'] = $request->method; //DD
        $income['ref_no'] = $request->reference_no; //Reference
        $income['date'] = Helper::ymd($request->purchase_date);
        $income['amount'] = bcdiv($request->amount_paid, 1, 2);
        $income['remaining'] = bcdiv($request->loan_amount, 1, 2);
        $income['remarks'] = $request->loan_about;

        IncomeExpense::updateOrCreate(['transaction_id' => $tt->id], $income);
        ...
        //Check transaction and income expnee
        // exit;
        return redirect('admin/vehicles/' . $request->get('vehicle_id') . '/edit?tab=acq-tab');
    }
}
