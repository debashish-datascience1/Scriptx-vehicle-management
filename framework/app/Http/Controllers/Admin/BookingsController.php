...
class BookingsController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        if (Helper::isGlobalSearch()) Session::put('global_redirects', ['booking_edit' => url()->previous()]);
        else Session::put('global_redirects', ['booking_edit' => url()->previous()]);
        // dd($index);
    ...
    public function modalcomplete(Request $request)
    {
        ...
        if (Helper::isGlobalSearch()) Session::put('global_redirects', ['booking_complete' => url()->previous()]);
        else Session::put('global_redirects', ['booking_complete' => url()->previous()]);
    ...
    public function modalroute(Request $request)
    {
        ...
        // dd($index);
        if (Helper::isGlobalSearch()) Session::put('global_redirects', ['booking_addroute' => url()->previous()]);
        else Session::put('global_redirects', ['booking_addroute' => url()->previous()]);
    ...
    public function addroute_save(Request $request)
    {
        ...
        $book->setMeta($index);
        $book->save();
        if (Session::has('global_redirects') && !empty(Session::get('global_redirects')['booking_addroute']))
            return redirect(Session::get('global_redirects')['booking_addroute']);
        else
            return redirect()->back();
    }
    ...
    public function lateadvanceSave(Request $request)
    {
        ...
        // $book->save();
        $book = Bookings::find($request->booking_id);
        $existingAdvance = $book->getMeta('advance_pay', 0); // Default to 0 if not set
        // Calculate new advance_pay by adding the new driver_advance
        $newAdvance = $existingAdvance + $request->driver_advance;
        // Update the meta data
        $data = ['advance_pay' => $newAdvance, 'advance_date' => $request->advance_date];
        $book->setMeta($data);
        $book->save();
    ...
    public function modal_save(Request $request)
    {
        ...
        if (Session::has('global_redirects') && !empty(Session::get('global_redirects')['booking_complete']))
            return redirect(Session::get('global_redirects')['booking_complete']);
        else
            return redirect()->back();
    }
