...
class ReportsController extends Controller
{
	...
	*/
	...
	{
		// Get vendors and fuel types for dropdowns
		$data['vendors'] = Vendor::where('type', 'fuel')->get();
		
		// Create vendor dropdown with "ALL" option
		$vendor_dropdown = ['all' => 'ALL'];
		foreach ($data['vendors'] as $vendor) {
			$vendor_dropdown[$vendor->id] = $vendor->name;
		}
		$data['vendors'] = $vendor_dropdown;
		
		// Get fuel types with "ALL" option
		$fuel_types = FuelType::pluck('fuel_name', 'id')->toArray();
		$data['fuel_types'] = ['all' => 'ALL'] + $fuel_types;
		
		return view('reports.fuel-purchase', $data);
	}

	public function fuelPurchasePost(Request $request)
	{
		// Get vendors and fuel types for dropdowns
		$data['vendors'] = Vendor::where('type', 'fuel')->get();
		$vendor_dropdown = ['all' => 'ALL'];
		...
		// Base query for fuel purchases - removed deleted_at condition
		$query = FuelPurchase::whereBetween('date', [$from_date, $to_date]);
	...
	public function printFuelPurchase(Request $request)
	{
		// Validate input
		$request->validate([
			'vendor_id' => 'required',
		...
		// Fetch records and calculate total
		$transactions = $query->get();
		$total_amount = $query->sum('amount');

		// Get fuel type name if specific type selected
		$fuel_type_name = 'ALL Types';
		if ($request->fuel_type !== 'all') {
			$fuel_type = FuelType::find($request->fuel_type);
			$fuel_type_name = $fuel_type ? $fuel_type->fuel_name : 'Unknown';
		}
		...
		return view('reports.print-fuel-purchase', $data);
	}
	...
	public function fuelTransport()
	{
		$data['transports'] = DB::table('group_transport')
		...
		return view('reports.fuel_transport', $data);
	}

	public function fuelTransportPost(Request $request)
	{
		$data['transports'] = DB::table('group_transport')
			->select('id', 'group_name')
			->pluck('group_name', 'id');
		...
		// Base query with vehicle relationship
		$query = FuelModel::with(['vehicle_data', 'group_transport', 'vendor', 'fuel_details']);

		if (!empty($transport_id) && !empty($fuel_type)) {
			$data['fuel'] = $query->where(['group_transport_id' => $transport_id, 'fuel_type' => $fuel_type])
				->whereBetween('date', [$start, $end])
				->orderBy("date", "DESC")
				->get();
		} else if (empty($transport_id) && !empty($fuel_type)) {
			$data['fuel'] = $query->where('fuel_type', $fuel_type)
				->whereNotNull('group_transport_id')
				->whereBetween('date', [$start, $end])
				->orderBy("date", "DESC")
				->get();
		} else if (!empty($transport_id) && empty($fuel_type)) {
			$data['fuel'] = $query->where('group_transport_id', $transport_id)
				->whereBetween('date', [$start, $end])
				->orderBy("date", "DESC")
				->get();
		} else {
			$data['fuel'] = $query->whereNotNull('group_transport_id')
				->whereBetween('date', [$start, $end])
				->orderBy("date", "DESC")
				->get();
		}
	...
	public function print_fuel_transport(Request $request)
	{
		$transport_id = $request->get('transport_id');
		$fuel_type = $request->get('fuel_type');
		...
		$data['transport_group'] = !empty($transport_id) ? DB::table('group_transport')->where('id', $transport_id)->first() : null;
		...
		return view('reports.print_fuel_transport', $data);
	}
	...
	public function cashBook_post(Request $request)
	{
		...
		$bookings = Bookings::whereDate('pickup', $date)
			->whereNull('deleted_at')
			->get();
		...
		$other_costs = DB::table('bookings')
			...
			->whereExists(function ($query) {
				$query->select(DB::raw(1))
					...
					->whereNull('deleted_at')  // Added for bookings_meta table
	...
	public function globalSearch()
	{
		...
		$data['request'] = $sessionArray;
		$data['param_post'] = null;
		// dd(Session::get('globalSearch'));
		// dd($data);
	...
	{
		// dd($request->all());
		Session::put('globalSearch', $request->all());
		$param_id = $request->param_id;
		...
		// Session::set('is_globalSearch', ['fuel_edit' => true]);
		// dd($data);
		return view('reports.global_search.global-search', $data);
	}
	...
	public function updateWheelPrices(Request $request)
	{
		try {
			DB::beginTransaction();
			
			foreach ($request->wheels as $wheelData) {
				DB::table('wheels')
					->where('id', $wheelData['id'])
					->update(['price' => $wheelData['price']]);
			}
			...
			return response()->json([
				'success' => true,
				'message' => 'Wheel prices updated successfully'
			]);
		} catch (\Exception $e) {
			DB::rollBack();
			...
			return response()->json([
				'success' => false,
				'message' => 'Error updating wheel prices: ' . $e->getMessage()
			], 500);
		}
	}

	public function getVehiclesAverage()
	{
		$averages = DB::table('vehicles_meta')
			->where('key', 'average')
			->whereNull('deleted_at')
	...
	public function updateAverages(Request $request)
	{
		try {
			$averages = $request->input('averages');
			
			foreach ($averages as $vehicleId => $average) {
	...
	public function getVehiclesFuelBalance(Request $request)
	{
		try {
			...
			foreach ($vehicles as $vehicle) {
				...
				$initialBalance = $fuelBalanceRecord ? floatval($fuelBalanceRecord->balance) : 0;
	...
	public function storeFuelBalanceRecords(Request $request)
	{
		try {
			...
			foreach ($vehicles as $vehicle) {
				...
				// Create or update fuel balance for next month
				// Use the current month's month_end_balance as the initial balance for next month
				$nextMonthFuelBalance = FuelBalance::updateOrCreate(
					[
						'vehicle_id' => $vehicle->id,
						'month' => $nextMonth,
						'year' => $nextYear
					...
					[
						'balance' => $remainingFuel,  // Use current month's end balance as next month's initial balance
						'month_end_balance' => NULL  // Initially set to the same value
					]
				);
	...
	public function vehicleOverview_post(Request $request)
	{
		...
		if ($request->get('vehicle_id') === 'all') {
            ...
            ->leftJoin('wheels', 'vehicles.wheel', '=', 'wheels.id')
			...
			foreach ($vehicles as $vehicle) {
				...
				$driver = DriverVehicleModel::where('vehicle_id', $vehicle->id)
					->first();
				$driver_salary = 0;
				if ($driver) {
				...
				// Get fuel data
				$fuelModel1 = FuelModel::where('vehicle_id', $vehicle->id)
					->whereBetween('date', [$startDateTime, $endDateTime])
					->whereNull('deleted_at')
					->get();
				...
				// 	$totalFuelQty += $f->qty;
				// }
				// Calculate total for other fuel types
				$otherFuelTotal = 0;

				foreach ($fuelModel1 as $f) {
					$fuelName = FuelType::find($f->fuel_type)->fuel_name;
					
					// Skip Diesel and Petrol
					if (in_array($fuelName, ['Diesel', 'Petrol'])) {
						continue;
					}
					...
					$otherFuelTotal += $f->qty * $f->cost_per_unit;
				}
			...
			$data['totalFuelCost'] = $currentTotalFuelCost;
		}
...
// 		dd($index);
	...
	public function vendorReport_post(Request $request)
	{
		...
		// dd($index);
		// dd($index['fuel']->pluck('vehicle_no'));
		return view('vendors.report', $index);
	}


	public function vendorReport_export(Request $request)
	{
		return Excel::download(new FuelExport($request), 'fuel-export.xlsx');
	}
