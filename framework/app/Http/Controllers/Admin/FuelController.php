...
class FuelController extends Controller
{
	...
	public function store(FuelRequest $request)
	{
		...
		$fuel->date = Helper::ymd($request->get('date'));
		...
		if (!empty($request->qty) && !empty($request->cost_per_unit)) {
			...
			$income['date'] = Helper::ymd($request->get('date'));
			$income['amount'] = 0;
	...
	public function update(FuelRequest $request)
	{
		...
		$fuel->save();
		if (!empty($request->qty) && !empty($request->cost_per_unit)) {
			$transactionData = Transaction::where(['param_id' => 20, 'from_id' => $request->id]);
			if ($transactionData->exists()) {
				$transactionData->update(['total' => $grandtotal]); //param_id = 20 fuel

				IncomeExpense::where('transaction_id', $transactionData->first()->id)->update(['date' => Helper::ymd($request->get('date')), 'remaining' => $grandtotal]);
			}
		}
		// VehicleModel::where('id', $request->vehicle_id)->update(['mileage' => $request->start_meter]);
		if (Session::has('global_redirects') && !empty(Session::get('global_redirects')['fuel_edit']))
			return redirect(Session::get('global_redirects')['fuel_edit']);
		else
	...
	public function destroy(Request $request)
	{
		...
		// if ($request->has('page')) return redirect()->route('reports.global-search', $sessionArray);
		// else return redirect()->route('fuel.index');
