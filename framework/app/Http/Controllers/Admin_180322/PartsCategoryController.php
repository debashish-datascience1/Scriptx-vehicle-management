...
class PartsCategoryController extends Controller
{
    ...
    {
        PartsCategoryModel::create(['name' => $request->name, 'is_itemno' => $request->is_itemno]);
    ...
    {
        PartsCategoryModel::where('id', $request->id)->update(['name' => $request->name, 'is_itemno' => $request->is_itemno]);
        return redirect()->route('parts-category.index');
    }
